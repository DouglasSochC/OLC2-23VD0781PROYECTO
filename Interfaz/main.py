...
from .utils.functionTransform import generate_create_function
    ...
    try:
        ...
        if carpeta_coincidente:
            ...
            if os.path.exists(carpeta_proc):
                ...
                messagebox.showwarning("Advertencia", "La carpeta 'Procedimientos no existe' no existe.")

            carpeta_proc = os.path.join(carpeta_principal, 'Funciones')
            if os.path.exists(carpeta_proc):
                # Crear una carpeta con el nombre del item seleccionado para almacenar los archivos SQL
                carpeta_sql = os.path.join(carpeta_principal, f'{nombre_item}_DUMP')
                os.makedirs(carpeta_sql, exist_ok=True)
                ...
                messagebox.showinfo("Éxito", f"Archivos SQL creados para Funciones en {carpeta_sql}")
            else:
